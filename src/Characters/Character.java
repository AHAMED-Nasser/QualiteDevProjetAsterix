...
public abstract class Character implements ICharacterAction {
    ...
    // dose of magic potion
    boolean temporaryStrength;// temporary strength
    boolean permanentStrength;// permanent strength
    boolean temporaryInvincibility;// temporary invincibility
    boolean permanentInvincibility; // permanent invincibility
    boolean graniteStatue;// turns into a granite statue
    boolean duplicationPower;// duplication power
    boolean metamorphosisPower;// metamorphosis power (lycanthrope)
    ...
    public void drinkPotion(MagicPotion potion) {
        if (this.isDead()) return;
        // règles de la potion magique

        // 1) Si le personnage boit une dose : force et invincibilité temporaires
        if (potion.getDoses() == 1 && potion.getCauldrons() == 0) {
            this.temporaryStrength = true;// force temporaire
            this.temporaryInvincibility = true;// invincibilité temporaire
            System.out.println(this.getName() + " boit une dose de potion magique : force et invincibilité temporaires !");
        }

        // 2) S'il boit une marmite : force et invincibilité permanentes
        if (potion.getCauldrons() == 1) {
            this.permanentStrength = true;// force permanente
            this.permanentInvincibility = true;// invincibilité permanente
            System.out.println(this.getName() + " boit une marmite entière : force et invincibilité permanentes !");
        }

        // 3) S'il boit deux marmites : transformation en statue de granit
        if (potion.getCauldrons() >= 2) {
            this.graniteStatue = true;// statue de granit
            System.out.println(this.getName() + " boit deux marmites : il devient une statue de granit !");
            return; // Les statues ne peuvent plus recevoir d'autres effets
        }

        // 4) Ingrédient : lait de licorne à deux têtes : pouvoir de dédoublement
        if (potion.hasTwoHeadedUnicornMilk()) {
            this.duplicationPower = true;// pouvoir de dédoublement
            System.out.println(this.getName() + " obtient le pouvoir de dédoublement !");
        }

        // 5) Ingrédient : poils d'Idéfix : pouvoir de métamorphose
        if (potion.hasIdefixHair()) {
            this.metamorphosisPower = true;// pouvoir de métamorphose
            System.out.println(this.getName() + " obtient le pouvoir de métamorphose !");
        }
    }
